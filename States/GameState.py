...
class GameState(State):
    ...
    def calculate_gold_reward(self, playerInfo, stage=0, total=0):
        if stage == 0:
            blind = playerInfo.currentBlind.upper()
            if blind == "SMALL":
                total += 4
            elif blind == "BIG":
                total += 8
            ...
            return self.calculate_gold_reward(playerInfo, stage=1, total=total)
            ...
            def overkill_bonus(points):
                if points <= 0:
                    return 0
                return 1 + overkill_bonus(points - 1)

            total += overkill_bonus(overkill)
    ...
    def SortCards(self, sort_by: str = "suit"):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                # --------- MODO ORDENAR POR SUIT PRIMERO ---------
                if sort_by == "suit":

                    if suitIndex(c1) > suitIndex(c2):
                        self.cards[i], self.cards[j] = self.cards[j], self.cards[i]

                    elif suitIndex(c1) == suitIndex(c2) and rankIndex(c1) > rankIndex(c2):
                        self.cards[i], self.cards[j] = self.cards[j], self.cards[i]

                # --------- MODO ORDENAR POR RANK PRIMERO ---------
                elif sort_by == "rank":
                    if rankIndex(c1) > rankIndex(c2):
                        self.cards[i], self.cards[j] = self.cards[j], self.cards[i]

                    elif rankIndex(c1) == rankIndex(c2) and suitIndex(c1) > suitIndex(c2):
                        self.cards[i], self.cards[j] = self.cards[j], self.cards[i]
    ...
    def discardCards(self, removeFromHand: bool):
        ...
        if not self.cardsSelectedList:
            ...
            if removeFromHand:
                needed = 8 - len(self.hand)
                if needed > 0:
                    new_cards = State.deckManager.dealCards(self.deck, needed)
                    self.hand += new_cards
        ...
        card_to_remove = self.cardsSelectedList[0]
